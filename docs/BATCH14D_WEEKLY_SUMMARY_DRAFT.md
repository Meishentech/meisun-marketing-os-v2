...
- 週期定位：14D 第一版預設使用「本週一到今天」，不先做完整日期選擇器。
- `approval_requests`：週報只讀取待決策資料作為摘要，不自動新增、修改或同步審核單。
- 匯出：不沿用既有只匯出 KPI 的 `exportCurrentSummary()`，需建立專用週報文字產生與匯出函式。
- UI：採獨立週報頁，不放在 modal；總經理與行銷總監都有入口，業務端 Phase 1 不顯示。

### 14D 前端實作狀態

- 已新增總經理與行銷總監的「週報摘要」導覽頁。
- 已新增 dashboard 入口卡，可進入週報頁。
- 已建立週報資料彙整 helper、週報文字產生器、專用複製與 `.txt` 匯出函式。
- 週報頁只讀既有資料，不寫入 `approval_requests` 或任何來源表。
- 本機檢查通過 `node --check assets/app.js` 與 `git diff --check`。
- 本機瀏覽器已驗證週報頁可開啟、週報文字可產生、複製按鈕會觸發成功提示；本環境 viewport override 未成功套用到 375px，手機截圖需交 Claude Code 另行實測。
