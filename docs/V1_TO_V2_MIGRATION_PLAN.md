...
- v1 行銷資源庫停用或隱藏 `delResource()` 刪除按鈕。
- v1 資源列表若能讀到 `deleted_at`，將已封存資源標示為「已由 v2 封存」或從 v1 列表排除。
- 文件化操作規則：資源新增 / 編輯 / 封存改到 v2，v1 不再刪資源。

不建議在此批直接修改其他 v1 刪除功能，避免一次動太多。
...
需要 Claude 先審查：外鍵 cascade 影響、v1 對封存欄位的相容性、行銷案子表是否也需同步封存。
