...
## 追加驗證：修正結果確認（2026-07-15，commit `6d96fa3`、`30394f5`）

三項問題都已處理，逐一核對如下：

### 4.1（待決策中心缺中間層）——已修正，做法正確

`decisionListSection()`、`approvalFlowSection()`、`approvalKpis()` 三個函式都補上了 `state.dataStatus === "live"` 的中間層分支，`approval_requests` 是空的時候會明確顯示「目前沒有待審核事項」（`decisionListSection` 甚至給了 `"ok"` 語氣的綠色標籤，而不是用會讓人誤會的紅/黃色），不會再落到寫死的示範資料。跟 Batch 1-3 建立的三層判斷模式完全一致，這個修正是對的。

`requestKpis()` 沒有一起加中間層，但重新檢查後**這其實不影響正確性**：它先檢查的是 `state.data.salesRequests.length`（系統裡全部需求單的總數，不是個人份），只要系統裡有任何一筆需求單（現在至少有測試資料），就會往下用 `visibleSalesRequests()` 算出的個人份數字，一個沒有自己需求單的業務會正確看到「需求總數：0」。只有在**整個資料庫完全沒有任何需求單**這種短暫的初期狀態，才會落到demo KPI——優先權低，可以晚點再補，不影響目前使用。

### 4.2（業務需求單資料範圍）——已修正，商業規則已拍板並寫入文件

`visibleSalesRequests()` 拿掉了「自己沒有需求單就 fallback 顯示所有人」的邏輯，改成單純回傳自己的需求單（可能是空陣列），空狀態訊息也做了區分（「你目前沒有自己提出的需求單」vs「系統裡完全沒有資料」）。這個決定也已經寫進 `docs/phase-1-mvp.md`（「業務端只顯示自己提出的 `sales_requests`，不 fallback 顯示其他業務的需求」），確認業務之間不會互相看到彼此需求單內容。

同時 `approval_requests.title`/`summary`/`amount`/`due_date` 是「送審當下快照、不跟來源同步」這件事，也依建議寫進了 `docs/phase-1-mvp.md`，避免之後有人誤以為是即時查詢結果。

### 5（`marketing_campaign_budget_items` 缺付款欄位）——已修正，Batch 5 可以開始

新增了 `sql/phase1_batch5_expense_prereq.sql`，補上 `payment_status text`、`payment_date date`（皆 nullable）+ 一個索引，純增量、不影響既有 `quote_status` 或任何既有資料。`docs/phase-1-mvp.md` 第 6 步也更新標註這是 Batch 5 的前置 migration。這個從最早 schema 審查就懸著的問題現在解決了，`all_expenses_overview` 建立時三張費用表加 `marketing_campaign_vendors` 應該可以正常對齊欄位。

**結論：Batch 4 的所有發現都已妥善處理，沒有殘留阻塞問題，Batch 5（產品知識庫 + 費用彙總）可以開始。**
