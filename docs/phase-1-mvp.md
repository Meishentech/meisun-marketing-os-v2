...
14. v1 → v2 搬遷治理 Batch 11
   ...
   - Batch 14A：v1 已停用 `delRisk()` / `delRiskUpdate()` 真刪除，避免刪除風險時 cascade 清掉追蹤紀錄；v1 風險新增 / 編輯暫時保留到 v2 風險管理驗收完成。
   ...
   - Batch 15A / 15B：已拍板並執行 v1 寫入入口凍結。V1 對 `marketing_resources`、`marketing_campaigns`、`marketing_campaign_tasks`、`marketing_campaign_budget_items`、`marketing_campaign_documents`、`marketing_campaign_risks`、`marketing_campaign_risk_updates`、`marketing_campaign_performance` 的新增 / 編輯入口停用；畫面按鈕 disabled，儲存 / 快速處理 / 行銷案排序函式直接顯示「此功能已移至 V2，請至新版行銷管理平台操作。」且不送 API。V1 保留列表、下載、匯出等查詢；核心資料新增、編輯與子項詳情請至 V2 查看與操作。
   - Batch 15C：已建立 `V2_PRODUCTION_SMOKE_TEST.md`，固定 production 部署後的角色、行銷案、任務、預算、文件、風險、成效、Channel、週報、文宣資源、知識庫、業務需求單、手機版與 V1/V2 邊界驗收流程。此文件是執行清單，不代表每次部署已自動完成 smoke test。
   ...
   - Batch 17B SQL：`sql/phase1_batch17b_association_lifecycle.sql` 已由 SQL Editor 套用到 live Supabase，smoke test 確認 24 個生命週期欄位與兩個 view 來源統計可讀。
   ...
   - Batch 22：新增「美的補助規則 / 申請檢核」第一版。SQL `sql/phase1_batch22_midea_subsidy_rules.sql` 將原廠 2026 補助申請與報銷 Excel 轉成 `midea_subsidy_rules` 39 筆規則資料，並替 `marketing_campaigns` 增加專案層級補助規則、替 `marketing_campaign_budget_items` 增加補助適用、規則、申請狀態、核銷狀態與缺件提醒欄位。前端在行銷案主檔、預算項目與「預算 / 補助 / 付款」頁接上規則選擇、補助檢核視窗與缺件摘要。第一版只做流程提醒與檢核，不自動建立補助審核單、不替代原廠正式核銷判定。
