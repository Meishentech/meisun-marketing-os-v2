...
14. v1 → v2 搬遷治理 Batch 11
   ...
   - Batch 14D 動工前草案：`BATCH14D_WEEKLY_SUMMARY_DRAFT.md`。建議第一版做即時計算週報摘要，不建 `marketing_campaign_weekly_summaries` 快照表、不接 AI、不做自動寄送；總經理與行銷總監可查看、複製與匯出週報，原始資料仍回到各行銷案詳情頁編輯。
