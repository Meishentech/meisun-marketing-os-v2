...
目的：確認 V1 對已由 V2 接手資料表的剩餘寫入入口，並把凍結範圍固定下來。

已拍板凍結 8 張表的 V1 新增 / 編輯入口：
...
- 上述 8 張表已由 V2 接手主要管理流程，V1 不再保留新增 / 編輯。
- `marketing_campaign_documents` 的編輯列最高優先，因為 V1 換檔或改資料會實際破壞 V2 廠商合作 / 文件引用判讀，不只是介面混淆。
- 公會、招標、新聞、文案、成功案例等未列入 8 張表的 V1 功能不納入本批凍結。

V1 讀取查詢與 V2 生命週期欄位盤點：

| 資料表 | V2 生命週期欄位 | V1 讀取是否過濾 | 判斷 |
| --- | --- | --- | --- |
| `marketing_resources` | `deleted_at` | 是，資源庫 / 對外素材 / Dashboard 已加 `deleted_at=is.null` | 已封存資源不再出現在 V1 可用素材。 |
| `marketing_campaigns` | `archived_at` | 否 | V2 已接手寫入，V1 本批凍結新增 / 編輯；讀取暫保留歷史可見，避免舊總表突然少資料。 |
| `marketing_campaign_tasks` | `cancelled_at` | 否 | V1 本批凍結寫入；已取消資料若仍顯示，視為讀取過渡期限制。 |
| `marketing_campaign_budget_items` | `cancelled_at` | 否 | V1 本批凍結寫入；已取消資料若仍顯示，視為讀取過渡期限制。 |
| `marketing_campaign_documents` | `archived_at` | 否 | V1 本批凍結新增 / 編輯 / 刪除；文件生命週期以 V2 為準。 |
| `marketing_campaign_risks` | `archived_at` | 否 | V1 本批凍結新增 / 編輯 / 刪除；風險生命週期以 V2 為準。 |
| `marketing_campaign_risk_updates` | `cancelled_at` | 否 | V1 本批凍結追蹤新增 / 編輯 / 刪除；追蹤生命週期以 V2 為準。 |
| `marketing_campaign_performance` | 無生命週期欄位 | 不適用 | V2 已接手新增 / 編輯；V1 本批凍結新增 / 編輯 / 刪除。 |

需請 Claude Code 複查：

- V1 目前哪些函式仍會寫入上述表。
- V1 寫入入口是否已全面停用，包含畫面按鈕與 console 直接呼叫函式。
- V1 讀取查詢是否仍會顯示 V2 已封存 / 已取消資料，並確認是否接受作為過渡期限制。
...
- 行銷案主檔：V1 新增 / 編輯停用。
- 任務 / 預算 / 文件：V1 新增 / 編輯停用。
- 風險 / 追蹤：V1 新增 / 編輯停用，Dashboard 快速處理一併停用。
- 成效資料：V1 新增 / 編輯 / 刪除停用。

第一版採「保守停用」：

- V1 保留列表、下載、匯出等查詢；核心資料新增、編輯與子項詳情請至 V2 查看與操作。
- V1 寫入按鈕 disabled。
