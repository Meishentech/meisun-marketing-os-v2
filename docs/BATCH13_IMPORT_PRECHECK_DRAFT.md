...
4. 只先停用 v1 `delDocument()`，因為 `marketing_campaign_documents` 已被 v2 廠商合作模組真實引用。
5. 文件刪除停用時，必須同時停止 `deleteStorageFile('campaign-documents', ...)`，避免只留下資料列但檔案被刪。
6. `delTask()` 與 `delBudgetItem()` 暫時不動，因為 v2 目前尚未管理任務 / 預算；提早停用會讓行銷總監在 Batch 13B 上線前沒有修正入口。
...
- 不開始 v2 任務 / 預算 / 文件 UI。
- 不停用 v1 任務 / 預算刪除，留到 Batch 13B v2 接手時再處理。
...
Claude Code 已產出 `CLAUDE_CODE_REVIEW_BATCH13A_DRAFT.md`，主要結論如下：

1. `marketing_campaign_documents` 已有 v2 依賴，Batch 13A 需要立即停用 v1 文件真刪除。
2. `marketing_campaign_tasks` 與 `marketing_campaign_budget_items` 目前仍由 v1 獨佔管理，Batch 13A 不應提早停用任務 / 預算刪除。
3. 兩支舊匯入腳本應加第二層破壞性旗標。
4. Batch 13B 命名建議：任務 / 預算用 `cancelled_*`，文件用 `archived_*`。
5. Batch 13B 費用彙總建議：預算項目取消後排除於 `all_expenses_overview`。
6. 文件 Storage 替換策略需在 Batch 13B 明確拍板，因為資源庫 Batch 9B 的既有先例是資料庫更新成功後清掉舊檔。
